...
1. `.app-artifacts/{session_id}/features/{feature_slug}/feature-spec.json`을 읽어 이 기능이 추가하는 `new_components`와 `new_endpoints`를 파악한다.
2. `.app-artifacts/{session_id}/arch.md`를 읽어 확정된 frontend 스택을 파악한다 (문맥 참조용).
3. `.app-artifacts/{session_id}/api-contract.json`을 읽어 전체 API 경로를 파악한다 (엔드포인트 경로 일치용).
4. **출력 루트를 `.app-artifacts/{session_id}/features/{feature_slug}/frontend/`로 사용한다.** (orchestrator가 사전 생성함)
5. feature-spec.json의 `new_components` 목록에 해당하는 컴포넌트/페이지/API 클라이언트/훅 파일만 생성한다. 엔드포인트 경로는 feature-spec.json(또는 api-contract.json)의 `path`와 정확히 일치해야 한다.
6. **`.app-artifacts/{session_id}/frontend/` 하위 기존 파일은 절대 읽지도 쓰지도 않는다.**
7. 기존 앱에 통합하려면 필요한 등록 코드(예: App.jsx에 새 라우트 추가)는 파일을 수정하지 않고, `features/{feature_slug}/frontend/INTEGRATION.md`에 안내 텍스트만 남긴다.
...
- **default 모드**: `.app-artifacts/{session_id}/frontend/` 하위 프론트엔드 소스 트리 (API 클라이언트, 컴포넌트, 라우팅, 상태관리, 의존성 매니페스트)
- **feature 모드**: `.app-artifacts/{session_id}/features/{feature_slug}/frontend/` 하위 신규 기능 컴포넌트 트리 + `INTEGRATION.md`
