...
├── apk/                           ← build-apk 실행 시 생성 (선택적)
│   ├── capacitor.config.json      ← apk-builder: Capacitor 앱 설정
│   ├── package.json               ← apk-builder: Capacitor 의존성
│   ├── .env.android               ← apk-builder: API URL 환경변수
│   ├── build-apk.sh               ← apk-builder: 수동 빌드 스크립트
│   ├── apk-build.md               ← apk-builder: 빌드 결과 보고서
│   └── android/                   ← apk-builder: Android 네이티브 프로젝트 (빌드 도구 있을 시)
...
13. **build-apk는 프론트엔드 코드를 Capacitor로 래핑하여 Android APK를 생성한다.** React/Vue 스택만 지원. Java JDK + Android SDK가 있으면 직접 빌드하고, 없으면 `build-apk.sh`와 설정 파일을 제공한다. APK에는 백엔드 코드가 포함되지 않으며 백엔드는 별도 서버로 운영된다.
...
/build-apk {session_id}               ← 프론트엔드를 Android APK로 패키징 (Capacitor)
```
